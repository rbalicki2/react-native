  ...
  if (
    ...
    try {
      window.__REACT_DEVTOOLS_APPEND_COMPONENT_STACK__ =
        parseBool(
          storage.getString(LOCAL_STORAGE_SHOULD_APPEND_COMPONENT_STACK_KEY),
        ) ?? true;
      window.__REACT_DEVTOOLS_BREAK_ON_CONSOLE_ERRORS__ =
        parseBool(
          storage.getString(LOCAL_STORAGE_SHOULD_BREAK_ON_CONSOLE_ERRORS),
        ) ?? false;
      window.__REACT_DEVTOOLS_SHOW_INLINE_WARNINGS_AND_ERRORS__ =
        parseBool(
          storage.getString(LOCAL_STORAGE_SHOW_INLINE_WARNINGS_AND_ERRORS_KEY),
        ) ?? true;
      window.__REACT_DEVTOOLS_HIDE_CONSOLE_LOGS_IN_STRICT_MODE__ =
        parseBool(
          storage.getString(LOCAL_STORAGE_HIDE_CONSOLE_LOGS_IN_STRICT_MODE),
        ) ?? false;
      window.__REACT_DEVTOOLS_BROWSER_THEME__ =
        parseBrowserTheme(storage.getString(LOCAL_STORAGE_BROWSER_THEME)) ??
        'dark';
    } catch {}
    try {
      window.__REACT_DEVTOOLS_GLOBAL_HOOK__.injectDeviceStorageMethods({
